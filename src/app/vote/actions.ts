...
export async function getRunoff(token: string) { return nameElectionService.getRunoffForInvitation(token); }
export async function saveRunoffChoice(token: string, suggestionId: number) { return nameElectionService.saveRunoffChoice(token, suggestionId); }
